...
export default function LeadCapturePopup({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ fullName: '', email: '', whatsappNumber: '', city: '', currentStatus: '' });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open: boolean) => {
      ...
      <DialogContent 
        className="sm:max-w-[500px] bg-white p-8 rounded-2xl border-0 shadow-2xl relative [&>button:first-child]:hidden"
        style={{ 
          zIndex: 9999,
          opacity: 1,
          visibility: 'visible',
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          <div>
            <input
              ...
              required
              className="w-full h-12 px-4 border-2 border-gray-300 rounded-lg text-base placeholder:text-gray-400 focus:outline-none focus:border-[#ff0000] focus:ring-0 transition-all"
          ...
          <div>
            <div className="relative">
              ...
              <input
                ...
                maxLength={10}
                pattern="[0-9]{10}"
                className="w-full h-12 pl-20 pr-4 border-2 border-gray-300 rounded-lg text-base placeholder:text-gray-400 focus:outline-none focus:border-[#ff0000] focus:ring-0 transition-all"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Current Status <span className="text-[#ff0000]">*</span>
            </label>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer px-4 py-3 rounded-lg border-2 border-gray-300 hover:border-[#ff0000] transition-all flex-1 bg-white">
                <input
                  type="radio"
                  name="currentStatus"
                  value="student"
                  ...
                  onChange={handleInputChange}
                  className="h-5 w-5 text-[#ff0000] border-gray-300 focus:ring-[#ff0000] cursor-pointer accent-[#ff0000]"
                  required
                />
              ...
              <label className="flex items-center gap-2 cursor-pointer px-4 py-3 rounded-lg border-2 border-gray-300 hover:border-[#ff0000] transition-all flex-1 bg-white">
                <input
                  ...
                  name="currentStatus"
                  value="professional"
                  checked={formData.currentStatus === 'professional'}
                  onChange={handleInputChange}
                  className="h-5 w-5 text-[#ff0000] border-gray-300 focus:ring-[#ff0000] cursor-pointer accent-[#ff0000]"
                  required
                />
          ...
          <button
            ...
            className="w-full h-12 bg-[#ff0000] text-white rounded-lg hover:bg-[#cc0000] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-[#ff0000] focus:ring-offset-2 font-semibold text-base mt-6"
          >
