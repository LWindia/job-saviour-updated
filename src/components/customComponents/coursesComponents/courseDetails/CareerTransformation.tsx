...
export default function CareerTransformation() {
  ...
  const [beforeLogoError, setBeforeLogoError] = useState(false);
  const [afterLogoError, setAfterLogoError] = useState(false);

  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % peopleData.length);
    setImageError(false); // Reset error state when changing person
    setBeforeLogoError(false);
    setAfterLogoError(false);
  };
  ...
  // Reset image error when person changes
  useEffect(() => {
    setImageError(false);
    setBeforeLogoError(false);
    setAfterLogoError(false);
                  ...
                  {!imageError ? (
                    ...
                    <div className="w-full h-[400px] bg-gradient-to-br from-gray-200 to-gray-300 rounded-lg flex items-center justify-center">
                      <div className="text-center">
                        <div className="w-24 h-24 rounded-full bg-[#ff0000] flex items-center justify-center mx-auto mb-4">
                ...
                <Card className="p-4 border border-blue-400 relative flex-1 min-w-[50px]">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-[60px] relative">
                      {!beforeLogoError ? (
                        <Image
                          src={typeof person.beforeLogo === 'string' ? encodeURI(person.beforeLogo) : person.beforeLogo}
                          alt="Before logo"
                          fill
                          className="object-contain"
                          onError={() => setBeforeLogoError(true)}
                          unoptimized={typeof person.beforeLogo === 'string' && person.beforeLogo.startsWith('http')}
                        />
                      ) : (
                ...
                <Card className="p-4 border border-green-500 relative flex-1 min-w-[100px]">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-[60px] relative">
                      {!afterLogoError ? (
                        <Image
                          src={typeof person.afterLogo === 'string' ? encodeURI(person.afterLogo) : person.afterLogo}
                          alt="After logo"
                          fill
                          className="object-contain"
                          onError={() => setAfterLogoError(true)}
                          unoptimized={typeof person.afterLogo === 'string' && person.afterLogo.startsWith('http')}
                        />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center bg-gray-100 rounded">
                          <span className="text-gray-400 text-xs">Logo unavailable</span>
