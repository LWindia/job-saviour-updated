...
const StudentCard = ({ employee, index }: { employee: any; index: number }) => {
  ...
  return (
    <div className={styles.slide}>
      <div className="border-2 rounded-lg shadow-lg bg-pink-100 h-[300px] flex flex-col items-center relative p-2">
        <a href={employee.linkding || '#'} target={employee.linkding ? "_blank" : "_self"} rel="noopener noreferrer" className="w-full h-full flex flex-col items-center">
          <div className="absolute top-1 right-1 bg-white p-1 rounded-full shadow-md text-sm">📌</div>
          
          <div className="flex flex-col items-center w-full mt-2">
            <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-[#ff0000] flex-shrink-0 bg-gray-200 flex items-center justify-center relative">
              {!imageError ? (
                <>
                  <img 
                    src={encodeURI(employee.photo)} 
                    alt={employee.name} 
                    className={`w-full h-full object-cover ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
                    onError={() => {
                      console.log('Image failed to load for:', employee.name, 'Path:', employee.photo);
                      setImageError(true);
                    }}
                    onLoad={() => {
                      setImageLoaded(true);
                  ...
                  {!imageLoaded && !imageError && (
                    <div className="absolute inset-0 flex items-center justify-center bg-gray-200">
                      <div className="w-4 h-4 border-2 border-[#ff0000] border-t-transparent rounded-full animate-spin"></div>
                    </div>
                  )}
                </>
              ) : (
                <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-red-500 to-red-700 text-white font-semibold text-xs">
          ...
          <div className="mt-2 flex justify-center items-center h-12">
            <img 
              src={encodeURI(employee.companyLogo)} 
              alt="Company Logo" 
              className="max-w-[100px] max-h-full object-contain"
              onError={(e) => {
                console.log('Company logo failed to load for:', employee.name);
...
const PlacedStudents = () => {
  const employees = [
            ...
            },
        {
          ...
          photo: "https://media.licdn.com/dms/image/v2/D4D03AQFOm1Et-F6_Zg/profile-displayphoto-shrink_400_400/profile-displayphoto-shrink_400_400/0/1696063474166?e=1768435200&v=beta&t=HvG3P6er0PO6tfybunVVloGn0XgXwoJVcg1KQAIU8oE",
                    ...
                    <div className={styles.slider}>
                      <div className={styles['slide-track']}>
                        {[...employees, ...employees].map((employee, index) => (
                          <StudentCard key={`${employee.name}-${index}`} employee={employee} index={index} />
                        ))}
                      </div>
                    </div>
                  </div>
